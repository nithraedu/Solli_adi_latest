...
import nithra.tamil.word.game.solliadi.biling_code.SkusAdapter;
...
public class UiManager implements RowViewHolder.OnButtonClickListener {
    ...
    public UiManager(int val,RowDataProvider rowDataProvider, BillingProvider billingProvider) {
    ...
    public void onBindViewHolder(SkuDetails data, RowViewHolder holder) {
        if (data != null) {
            ...
            if (!data.getType() .equals( SkusAdapter.TYPE_HEADER)) {
    ...
    public void onButtonClicked(int position) {
        SkuDetails data = mRowDataProvider.getData(position);
        if (data != null) {
