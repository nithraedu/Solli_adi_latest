...
public class WordsearchGridView extends LinearLayout implements OnKeyListener {
    ...
    final ArrayList<String> wordfounted_color = new ArrayList<>();
    final ArrayList<String> wordfounted_word = new ArrayList<>();
    ...
    public void wordFound(Word word) {
        if (!mGameState.mFoundWords.contains(word)) {
            ...
            // WordsearchGridFragment.lay_wordsearch.addView(tv);
            ...
            new Handler(Looper.myLooper()).postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    WordsearchGridFragment.lay_wordsearch.removeView(tv);
                }
            }, 1000);
            ...
            wordfounted_color.add(select_color);
            wordfounted_word.add(word.getWord());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mGameState.foundWords.size() > 0) {
            ...
            if (wordfounted_color.size() < mGameState.foundWords.size()) {
                for (Word mword : mGameState.foundWords) {
                    String find_color = colors[new Random().nextInt(colors.length)];
                    wordfounted_color.add(find_color);
                    wordfounted_word.add(mword.getWord());
                }

                System.out.println(" call me foundWords : " + mGameState.foundWords.size());
                System.out.println(" call me onDraw wordfounted_color : " + wordfounted_color.size());
            }
            ...
            for (Word mword : mGameState.foundWords) {
                ...
                System.out.println("^^^^^^^^^^^^^^^^^^^^^^^^^^ =if (mGameState.foundWords.size() > 0) " + mword);
                ...
                String find_color = wordfounted_color.get(go);
                go += 1;

                mMtPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
                mMtPaint.setColor(Color.parseColor(find_color));
                drawSelection(mword, canvas, mMtPaint);

                System.out.println(" call me mword : " + mword);
                System.out.println(" call me find_color " + go + " : " + find_color);
